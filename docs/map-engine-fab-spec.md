...
- The custom runtime remains a development preview behind the feature flag until popup, hover, and selection parity are consistently verified.
...
These are still adapter-backed:

| Need | Status | Runtime coverage |
| --- | --- | --- |
| GraphHopper on-map routing | adapter-backed | leaflet-adapter only |
| live geolocation controls | adapter-backed | leaflet-adapter only |
...
- GraphHopper route rendering
- browser geolocation workflows
- admin editing tools
...
- Leaflet is a compatibility adapter and current production baseline, not the long-term platform boundary.
...
The custom runtime should remain behind the feature flag until the adapter-only
boundaries are either removed or intentionally accepted and the interaction
parity bugs are no longer part of normal development.
